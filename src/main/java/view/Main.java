...
import model.DB_Utilities.*;
import model.*;
import model.Class;
import model.tools.ReadCSV;
import model.tools.ReadFile;
import model.tools.Serializer;
...
//    static boolean friExclFlag;
    ...
    public static void main(String[] args)  {
        ...
        //READ CSV FILE INTO an array of CoursePlannerBIGGEST_Entry objects. (i.e. coursePlannerBIGGESTEntryList)

        List<CoursePlannerBIGGEST_Entry> coursePlannerBIGGESTEntryList = new ArrayList<>();
...
//                    lblEndTime.setText("Ends at: " + curTimeEnd);
        ...
        List<CourseSubject_Catalog_Tuple> tupleList = new ArrayList<CourseSubject_Catalog_Tuple>();
        List<List<model.Class>> classesList = new ArrayList<>();
...
//                int selectedPriority = (int) priorityValuesComboBox.getSelectedItem();
                ...
                CourseSubject_Catalog_Tuple courseTuple2BeAdded = new CourseSubject_Catalog_Tuple(
                        selectedCourseSubject,
                        selectedCourseCatalog);
//                        ,selectedPriority);
...
//                    courseNamesInPlanningList.add(selectedCourseName);
                    ...
                    for (int i = 0; i < classesList.size(); i++) {// CHECKING for ELEC317 vs. COMP317 case. (i.e. courses with same classes)
                        List<model.Class> curClassList = classesList.get(i);
                        for (model.Class curClass : classListForTuple) {
                            if (curClassList.contains(curClass)) {
                                CourseSubject_Catalog_Tuple tupleAlreadyAdded = tupleList.get(i);
...
//                }
        ...
        lstCourse2BePlanned.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                CourseSubject_Catalog_Tuple selectedTuple = (CourseSubject_Catalog_Tuple) lstCourse2BePlanned.getSelectedValue();
                ...
                Thread threadExportAsHtml = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ExportAsHTML(finalRecordName, lblProgressBar);
                            SetProgressBarLabelTxt(lblProgressBar, "Status: ", "<font face=\"verdana\" color=\"green\"><b><i>", "idle");
                            File htmlFile = new File(savePath + "Plans for " + finalRecordName + ".html");
                            OpenHTML(htmlFile);
                        } catch (IOException e1) {
                            e1.printStackTrace();
                        }
...
//                    String selectedItemStr = (String) scheduleListComboBox.getSelectedIndex();
    ...
    private static void ExportAsJson(String finalRecordName, List<CourseSubject_Catalog_Tuple> tupleList, DefaultListModel lstAddedClassFiltersListModel, DefaultListModel lstFiltersModel, JLabel lblProgressBar) throws IOException {
    ...
    private static String GetFinalRecordName(DefaultListModel lstCourses2BePlannedModel) {
        ...
        for(int i=0;i<lstCourses2BePlannedModel.getSize();i++){
            CourseSubject_Catalog_Tuple curTuple = (CourseSubject_Catalog_Tuple)lstCourses2BePlannedModel.getElementAt(i);
    ...
    private static void ExportSchedulesAsImages(String savePath, List<JButton> btnList, List<Schedule> schedules, JLabel lblProgressBar) throws IOException, AWTException {
        WeeklyScheduleTimetable wsGUI = new WeeklyScheduleTimetable(btnList);
    ...
    private static void CreateAndFill_DB_from_CSV(List<CoursePlannerBIGGEST_Entry> coursePlannerBIGGESTEntryList, String fPath, String sqlQuery_Create_Location, String sqlQuery_Insert_Location, String dbName) {

        ReadCSV.FillIntoList(fPath, coursePlannerBIGGESTEntryList);
        //System.out.println(coursePlannerBIGGESTEntryList);
